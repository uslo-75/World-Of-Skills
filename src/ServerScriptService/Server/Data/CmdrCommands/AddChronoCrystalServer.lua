...
return function(_context, playersArg, amount)
	...
	for _, player in ipairs(players) do
		local ok, err = CmdrAdminUtil.AdjustCurrency(player, "ChronoCrystal", "add", amount)
		if ok then
			changed += 1
		else
	...
	return CmdrAdminUtil.FormatSummary("ChronoCrystal ajoutes", changed, #players, failed)
end
