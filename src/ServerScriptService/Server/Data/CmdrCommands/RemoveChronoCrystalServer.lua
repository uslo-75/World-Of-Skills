...
return function(_context, playersArg, amount)
	...
	for _, player in ipairs(players) do
		local ok, err = CmdrAdminUtil.AdjustCurrency(player, "ChronoCrystal", "remove", amount)
		if ok then
			changed += 1
		else
	...
	return CmdrAdminUtil.FormatSummary("ChronoCrystal retires", changed, #players, failed)
end
