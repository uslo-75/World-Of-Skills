--[[
	...
	Send a humanoid object and stun duration (seconds) to the function
	...
	for humanoid, data in pairs(Stunned) do
		...
		if currentTime >= data.duration then
			...
			if humanoid:IsDescendantOf(workspace) then
				...
				if data.jumpHeight == 0 then
					humanoid.JumpHeight = 6.2
				else
					humanoid.JumpHeight = data.jumpHeight or 6.2
				end

				humanoid.Parent:SetAttribute("Attacking", false)
				humanoid.Parent:SetAttribute("Swing", false)
				humanoid.Parent:SetAttribute("iFrames", false)
				humanoid.Parent:SetAttribute("UsingMove", false)
				humanoid.Parent:SetAttribute("Stunned", false)
				humanoid.Parent:SetAttribute("DoingStuff", nil)
				humanoid.Parent:SetAttribute("SlowStunned", false)
			end
		end
	...
	Stun = function(humanoid, duration)
		...
		if not data.changedConn then
			data.changedConn = humanoid.Changed:Connect(function()
				-- speed when stunned
				if humanoid.Parent:GetAttribute("SlowStunned") then
